...
function wishListForm() {
    ...
    app.getForm(request.triggeredForm.formId).handleAction({
        ...
        deleteItem: function (formgroup, action) {
            ...
            if (sessionCustomer.ID === action.object.list.owner.ID) {
        ...
        setListPrivate: function () {
            // No need to check for productList ownership at this point because the productList
            // retrieved from the shopper's profile and in the currently implementation the shopper
            // can only have on productList of type TYPE_WISH_LIST
        ...
        selectAddressWishlist: function (form, action) {
            var sessionCustomer = session.customer;
            if (sessionCustomer.ID === action.object.list.owner.ID) {
                setShippingAddress();
            }
...
function showOther() {
    var URLUtils = require('dw/web/URLUtils');
    var wishlistForm = app.getForm('wishlistpublic');
    wishlistForm.clear();
    ...
    }).render('account/wishlist/wishlistpublic');
}
