...
function registrationForm() {
    app.getForm('profile').handleAction({
        confirm: function () {
            var email, emailConfirmation, orderNo, orderUUID, profileValidation, password, passwordConfirmation, existingCustomer, Customer, target;
            ...
            orderUUID =  app.getForm('profile.customer.orderUUID').value();
            ...
            if (profileValidation) {
                ...
                if (orderNo && orderUUID) {
                    var order = OrderMgr.getOrder(orderNo);
                    if (order && customerID === order.customer.ID && order.getUUID() === orderUUID) {
                        Transaction.wrap(function(){
                            order.setCustomer(profileValidation);
                        })
